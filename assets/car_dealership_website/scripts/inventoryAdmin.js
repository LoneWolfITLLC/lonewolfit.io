...
window.addEventListener("authChecked", async function () {
  ...
  function createCard(car) {
    ...
    deleteBtn.className = "inventory__delete-btn";
    deleteBtn.addEventListener("click", async (e) => {
      e.stopPropagation();
      confirmModal(
        `Are you sure you want to delete the vehicle: ${car.year} ${car.make} ${car.model}? This action cannot be undone.`,
        async (confirmed) => {
          if (confirmed) {
            try {
              await deleteVehicle(canonicalId);
              loadInventory(); // re-fetch and re-render
            } catch (error) {
    ...
    more.textContent = "Modify Details ›";
    ...
    card.addEventListener("click", () => {
      ...
      window.location.href = `edit_car.html?id=${encodeURIComponent(car.id)}`;
    });
  ...
  async function deleteVehicle(canonicalId) {
    const token = sessionStorage.getItem("jwt");
    if (!token){
      alertModal("Invalid session. Cannot delete vehicle. Please sign in again.");
      return;
    } 
    const res = await fetch(
      `${URL_BASE}/api/admin/cars/delete-listing/${encodeURIComponent(
        canonicalId
      )}`,
      {
        method: "DELETE",
    ...
    if (!res.ok) {
      const errText = await res.text().catch(() => "Failed to delete vehicle.");
      let errorJson = {};
      try {
        errorJson = JSON.parse(errText);
      } catch (err) {}
      alertModal(`Error deleting car: ${errorJson.message}`);
    }
  }
