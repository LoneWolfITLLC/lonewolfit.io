...
async function fetchUserDetailsWithoutToken(userId) {
    try {
        const response = await fetch(URL_BASE+'/api/auth/get-user-details/withouttoken', {
            method: "POST",
...
async function displayErrorSlide() {
    ...
    slides.innerHTML = '<div class="slideshow__slide slideshow__slide--blank"><p style="color:#0e0e0e;">Error loading available testimonials, please try again later or consult your technician...</p></div>';
}

async function displaySlides() {
    const slides = document.querySelector('.slideshow');
    slides.innerHTML = '<div class="slideshow__slide slideshow__slide--blank"><p style="color:#0e0e0e;">No testimonials available, please try again later...</p></div>';
    const slideIndices = [];
    ...
    if (testimonials.length === 1) {
        ...
        slideIndices.push(currentCenterSlide, (currentCenterSlide + 1) % testimonials.length);
    ...
    const createSlideContent = async (testimonial, index) => {
        ...
        slide.className = twoSlides ? 'slideshow__slide slideshow__slide--dual' : 'slideshow__slide';
...
function changeSlide(direction) {
    if (testimonials.length > 1 && !changingSlide) {
        ...
        setTimeout(() => {
            currentCenterSlide = (currentCenterSlide + direction + testimonials.length) % testimonials.length;
            displaySlides();
        }, 250);
    }
}
    ...
    try {
        ...
        if (token) {
            ...
            if (!response.ok) {
                ...
                response = await fetch(`${URL_BASE}/api/testimonials/approved`, { method: "GET" });
            }
        } else {
            response = await fetch(`${URL_BASE}/api/testimonials/approved`, { method: "GET" });
        }
