    ...
    if (
      ...
      fullUserData.then((data) => {
        ...
        if (data.address) {
          ...
          document.getElementById("countryBusiness").dispatchEvent(new Event("change"));
        ...
        document.getElementById("usernameResidential").value = data.username || "";
...
function getUserAddress(formType) {
  let address1 = "", address2 = "", city = "", state = "", zip = "", country = "";
...
function getBusinessAddress() {
  const address1 = document.getElementById("businessAddress1Business").value.trim();
  const address2 = document.getElementById("businessAddress2Business").value.trim();
  ...
  fetch(URL_BASE + "/api/auth/edit-user", {
    ...
    .then(async (response) => {
      ...
      if (response.ok) {
        ...
        alert(
          "Error updating user: " + (errorText || response.statusText)
        );
      }
    })
    .catch((err) => {
